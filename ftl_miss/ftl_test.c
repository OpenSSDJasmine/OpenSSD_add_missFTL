...
static void tc_write_rand_flash (const UINT32 start_lsn, const UINT32 io_num, const UINT32 sector_size);
...
void ftl_test(void)
{
	...
	ptimer_start();
	tc_write_rand(0, 50, NUM_PSECTORS_4KB);
	ptimer_stop_and_uart_print();
/*
	ptimer_start();
	tc_write_rand_flash(0, 50, NUM_PSECTORS_4KB);
	ptimer_stop_and_uart_print();
	*/
/*     fillup_dataspace(); */
...
static void tc_write_rand_flash(const UINT32 start_lsn, const UINT32 io_num, const UINT32 sector_size)
{
	...
	for (UINT32 loop = 0; loop < 1; loop++) {
		...
		for (i = 0; i < io_cnt; i++) {
			...
			rd_buf_addr = RD_BUF_PTR(g_ftl_read_buf_id) + ((lba % SECTORS_PER_PAGE) * BYTES_PER_SECTOR);
			/*             ptimer_start(); */
			ftl_read_flash(lba, num_sectors);
			/*             ptimer_stop_and_uart_print(); */
			//uart_printf("read fin");
			flash_finish();
